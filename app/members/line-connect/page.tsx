  ...
  useEffect(() => {
    ...
    const performServerSideOperations = async (code) => {
      try {
        ...
        const response = await apiClient.lineConnect(code, redirectUri);

        if (response.success) {
          router.push('/'); // Redirect to profile page on success
        } else {
          const errorData = await response.json();
          setErrorMessage(errorData.message || 'Connect failed');
          setTimeout(() => {
            router.push('/members/login');
          }, 3000);
        }
      } catch (error) {
        console.error("Error during line connect:", error);
